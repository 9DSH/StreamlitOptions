...
#-------------------------------------------------------------
        ...
        with main_tabs[0]:
            ...
            with filter_row:
                ...
                with col_range:
                    strike_range = st.slider(
                                            ...
                                            max_value=400000,
                                            step=500,
                                            value=(30000, 400000)  # Default range
                                        )
                with col_strike:
            ...
            if not market_screener_df.empty:
                ...
                with tabs[1]:  
                    padding, cal1, cal2,cal3 = st.columns([0.02, 0.7, 0.2 ,0.6])
                    ...
                    with cal1: 
                        ...
                        most_traded_options = get_most_traded_instruments(filtered_df)
...
if __name__ == "__main__":
    ...
    #if 'public_trades_thread' not in st.session_state:
      #  st.session_state.public_trades_thread = None
    ...
    #if st.session_state.public_trades_thread is None or not st.session_state.public_trades_thread.is_alive():
        # Start the options data refresh thread
       # st.session_state.public_trades_thread = threading.Thread(target=analytics.start_analyzing_public_trades)
       # st.session_state.public_trades_thread.start()
    
    
    app()  # Run the Streamlit application