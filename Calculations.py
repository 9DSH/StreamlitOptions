...
def get_most_traded_instruments(df):
    """
    Returns the three most traded instruments and their total sizes as formatted strings.
    """
    # Group by 'Instrument' and sum the 'Size'
    grouped_data = df.groupby('Instrument')['Size'].sum().reset_index()

    # Sort the DataFrame by total size in descending order and select the top 3
    most_traded = grouped_data.sort_values(by='Size', ascending=False).head(10)
    # Create a list of formatted strings for display
    formatted_results = [f"{row['Instrument']} --- contracts: {row['Size']}" for index, row in most_traded.iterrows()]
